...
- **인증 흐름**: 
  ...
  3. **토큰 자동 갱신 및 충돌 방지**: `get_refreshed_token()` 로직을 통해 토큰 만료 전 `refresh_token`을 사용하여 자동으로 `access_token`을 갱신합니다. 이때 `fcntl`을 이용한 파일 락(File Locking)을 도입하여 여러 워커 프로세스가 동시에 토큰을 갱신할 때 발생하는 데이터 충돌과 Gunicorn 워커 다운 현상을 원천 차단합니다.
- **로컬 데이터베이스 및 상시 동기화 (UPDATED)**:
  - **SQLite DB**: `mstodo.db`를 사용하여 모든 할 일 목록과 태스크를 로컬에 저장합니다.
  - **Background Sync Worker**: `sync_worker.py`가 도입되어 사용자가 접속하지 않더라도 1시간마다 백그라운드에서 자동으로 Microsoft Graph API와 동기화를 수행합니다. 이를 통해 며칠 만에 접속해도 토큰이 만료되지 않고 항상 최신 데이터를 즉시 로드할 수 있습니다.
  - **Delta Query Sync**: 변경된 데이터만 가져오며, 델타 토큰 만료 시 자동으로 전체 동기화를 수행합니다.
- **모든 할 일 (All Tasks)**:
  - 사이드바 상단에 '모든 할 일' 메뉴를 추가하여 모든 리스트의 할 일을 한눈에 모아볼 수 있습니다.
  - 서비스 로고 클릭 시 '모든 할 일' 화면으로 즉시 이동합니다.
- **리스트 그룹화 및 영속성**: 
...
- **완료된 할 일 관리 (NEW)**:
  - 완료된 할 일 목록의 폴딩(접기/펴기) 상태가 브라우저의 `localStorage`에 저장되어 페이지를 새로고침하거나 다시 방문해도 유지됩니다.
  - 완료된 할 일은 **완료된 시간 역순(최근 완료된 항목이 위로)**으로 정렬되어 최신 작업 내역을 쉽게 확인할 수 있습니다.
...
- **UI 강조 및 레이아웃**:
  ...
  - **애니메이션 및 배경**: 
    - 배경의 동적인 블롭 효과를 강화하여 세련된 UI를 제공합니다.
    - **실시간 날씨 효과 (Seoul)**: 서울의 실시간 날씨(맑음, 흐림, 비, 눈, 안개, 천둥번개 등)에 맞춰 화면에 다양한 애니메이션 효과를 표시합니다.
  - **UI 커스터마이징 (Settings)**: 
    - 설정 페이지에서 '배경 애니메이션'과 '실시간 날씨 효과'를 사용자의 취향에 따라 개별적으로 켜고 끌 수 있습니다.
    - 설정값은 브라우저의 `localStorage`에 저장되어 영구적으로 유지됩니다.
