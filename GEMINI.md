...
- **로컬 데이터베이스 및 동기화 (NEW)**:
  - **SQLite DB**: `mstodo.db`를 사용하여 모든 할 일 목록과 태스크를 로컬에 저장합니다. 이를 통해 페이지 로딩 속도가 비약적으로 향상되었습니다.
  - **Delta Query Sync**: Microsoft Graph API의 델타 쿼리 기능을 사용하여 변경된 데이터만 가져옵니다. 
    - 화면 진입 시 DB 데이터를 즉시 표시하고, 백그라운드에서 동기화를 수행하여 최신 상태를 반영합니다.
    - 델타 토큰이 만료(410 Gone)된 경우 자동으로 전체 동기화를 수행하여 데이터 정합성을 유지합니다.
- **모든 할 일 (All Tasks)**:
