...
- **중요도 우선 정렬**: 할 일 목록 조회 시, 중요도(`importance`)가 `high`인 항목을 최우선으로 정렬하여 사용자에게 중요한 일을 먼저 보여줍니다. (백엔드 및 프론트엔드 공통 적용)
- **Optimistic UI 정렬**: 중요도(별표) 토글 시 서버 응답을 기다리지 않고 즉시 로컬 데이터를 재정렬하여 반응성을 극대화했습니다. 실패 시에만 원복합니다.
- **제목 전체 표시**: 할 일 제목이 길어져도 생략(`...`)되지 않도록 `truncate` 클래스를 제거하고 전체 내용을 표시합니다.
