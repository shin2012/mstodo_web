...
- **전체 할 일 목록별 그룹화**: '전체 할 일' 조회 시 각 항목을 할 일 목록(List)별로 구분하여 헤더와 함께 표시함으로써 가독성을 개선했습니다.
- **수동 업데이트 버튼 적용**: 불필요한 API 호출을 방지하고 사용자가 원하는 시점에 데이터를 갱신할 수 있도록 기존의 자동 리프레시 기능을 수동 버튼 방식으로 대체했습니다.
